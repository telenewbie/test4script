...
def checklog(env, checkString):
    ...
    writeLog(env, "checklog: {0} <<>> {1}".format(env['syslogpath'], os.listdir(env['syslogpath'])))
    writeLog(env, sorted(os.listdir(env['syslogpath']), reverse=True))
